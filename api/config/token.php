...
define('SECRET_KEY','Your-Secret-Key12345');
...
class Token
{
    ...
    public static function create_token($userID,$username, $tip)
    {
    ...
    public static function create_token_resetPass($userID,$username)
    {
        $configs = include('config.php');
        $tokenId    = 55;
        $issuedAt   = time();
    ...
    public static function is_validA($token)
    {
        try
        {
            $decoded = JWT::decode($token, SECRET_KEY, array(ALGORITHM));
